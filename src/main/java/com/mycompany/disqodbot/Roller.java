...
public class Roller {
    ...
    String[] parseRollRequest(String request, String username) throws ExcessiveNumberOfDiceException {
        ...
        if((request).matches("((\\d)+d((\\d)+|f)|advantage|disadvantage|dorgan)([\\+\\-](\\d)+d((\\d)+|f)){0,8}([\\+\\-](\\d+))?")){
            //No trailing text
            //DEBUG
            ...
            Pattern pattern = Pattern.compile("((\\d)+d((\\d)+|f)|advantage|disadvantage|dorgan)([\\+\\-](\\d)+d((\\d)+|f)){0,8}([\\+\\-](\\d+))?");
            Matcher matcher = pattern.matcher(request);
            matcher.find();
            rollRequest = matcher.group();
            trailingText = request.replaceFirst("((\\d)+d((\\d)+|f)|advantage|disadvantage|dorgan)([\\+\\-](\\d)+d((\\d)+|f)){0,8}([\\+\\-](\\d+))?", "");
        }
        ...
        while (j<expressions.length) {
            ...
            } else if (expressions[j].equals("dorgan")) {
                //dorgan (fail) roll
                int[] results = new int[1];
                results[0] = 1;
                
                responseString+=("Dorgan roll: [1] ");
                responseValue+=multipliers[j];
